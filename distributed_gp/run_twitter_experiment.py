...
def run():
    ...
    end_of_time = 1354320000 + 7*24*3600  + 1*24*3600
    days_passed = 0
    _results =  {} 
    _saved = {}
    ...
    while clock<end_of_time:
        ...
        for i in range(1):
            ...
            try:
                gp = GaussianProcessJob( test_region, str(fourteen_days_ago), str(clock) , redis_queue, 'twitter')
        ...
        save_to_mongo(_results, _saved, cur_utc_timestamp, 'twitter') 
        clock+=3600*24
    done = False
    while not done:
