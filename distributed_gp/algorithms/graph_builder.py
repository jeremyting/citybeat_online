...
class EventGraph():
    ...
    def _locationDistance(self, location_a, location_b):  #compute distance of two lat/lng pairs
        return math.sqrt(
            (location_a[0] - location_b[0]) * (location_a[0] - location_b[0]) + (location_a[1] - location_b[1]) * (
    ...
    def doEventModularity(self, event):
        ...
        for node_a in G.nodes():
            for node_b in G.nodes():
                if node_a < node_b:
                    ...
                    dis_max = max(dis_max, self._locationDistance(self._getLocationPair(node_mapping[node_a]),
                                                                  self._getLocationPair(node_mapping[node_b])))

        for node_a in G.nodes():
            for node_b in G.nodes():
                if node_a < node_b:
                    G[node_a][node_b]['weight'] = 0.5 * (
                    1 - self._locationDistance(self._getLocationPair(node_mapping[node_a]),
                                               self._getLocationPair(node_mapping[node_b])) * 1.0 / dis_max )
        ...
        for node_a in G.nodes():
            for node_b in G.nodes():
                if node_a < node_b:
                    time_interval_max = max(time_interval_max, abs(
                        self._getTime(node_mapping[node_a]) - self._getTime(node_mapping[node_b])))
            #print 'time_max = ',time_interval_max
        for node_a in G.nodes():
            for node_b in G.nodes():
                if node_a < node_b:
                    time_interval = 1 - abs(
                        self._getTime(node_mapping[node_a]) - self._getTime(node_mapping[node_b])) * 1.0 / (
                                    time_interval_max + 1)
